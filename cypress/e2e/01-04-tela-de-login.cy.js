...
describe('2 - [TELA DE LOGIN] Crie o botão de iniciar o jogo', () => {
  ...
  it('Será validado se ao clicar no botão "Play" o jogo é iniciado salvando um token de jogador e redirecionando para a tela de jogo', () => {
    cy.get(INPUT_PLAYER_NAME_SELECTOR).type(PLAYER_NAME);
    cy.get(INPUT_PLAYER_EMAIL_SELECTOR).type(PLAYER_EMAIL);
    ...
    cy.window().its('localStorage.token').should((token) => {
...
describe('4 - [TELA DE LOGIN] Desenvolva testes para atingir 90% de cobertura da tela de Login', () => {
  it('Verifica a cobertura de testes unitários', () => {
    ...
    cy.getCoverage().its('Login.lines.pct', { timeout: 0 }).should('be.gte', 90.00);
  });
});
